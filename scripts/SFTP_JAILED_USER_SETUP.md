...
│   ├── file2.pdf
│   └── document.csv
└── [internal SSH structures] # Auto-generated by SSH for SFTP functionality
    └── dev/                  # Auto-created by SSH internal-sftp subsystem
```
...
To enhance security by requiring SSH keys instead of passwords:

1. **First, ensure users have generated SSH keys:**
   ```bash
   # Users should run this on their local machine
   ssh-keygen -t ed25519 -C "sftp_user@blast-engine"
   ```

2. **Set up authorized_keys for the jailed user:**
   ```bash
   sudo mkdir -p /data/sftp/.ssh
   sudo touch /data/sftp/.ssh/authorized_keys
   sudo chmod 700 /data/sftp/.ssh
   sudo chmod 600 /data/sftp/.ssh/authorized_keys
   
   # Add user's public key
   cat ~/.ssh/id_ed25519.pub | sudo tee -a /data/sftp/.ssh/authorized_keys
   ```

3. **Update SSH configuration to disable password auth:**
   ```bash
   sudo tee -a /etc/ssh/sshd_config.d/99-sftp-jail-sftp_user.conf << EOF
...
**Warning:** Only apply this after users have confirmed their keys work, or they will be locked out.
