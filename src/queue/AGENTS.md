# src/queue - Job Queue & Rate Limiting

**Purpose:** Background job processing with retry policies, rate limiting, and BullMQ (optional) support.

## STRUCTURE
```
queue/
â”œâ”€â”€ job-queue.ts          # Main JobQueue class (EventEmitter-based, BullMQ fallback)
â”œâ”€â”€ rate-limiter.ts       # Per-platform rate limit tracking
â”œâ”€â”€ retry.ts              # Retry/backoff logic
â”œâ”€â”€ retry-policies.ts     # Platform-specific retry policies
â”œâ”€â”€ job-worker.ts (../workers) # Consumer that processes jobs via adapters
```

## KEY FILES

| File | Lines | Purpose |
|------|-------|---------|
| `job-queue.ts` | ~204 | In-memory queue with optional BullMQ; emits completed/failed events |
| `rate-limiter.ts` | â€” | Token-bucket rate limiter per platform |
| `retry.ts` | â€” | Exponential backoff + jitter |
| `retry-policies.ts` | â€” | Platform-specific: max attempts, backoff base, retryable errors |

## CONVENTIONS

- `JobQueue extends EventEmitter` â€” consume via `queue.on('completed', ...)`
- Uses `async-mutex` Mutex (not boolean flag) for atomic processing (FIX #2)
- In-memory queue for dev/testing; BullMQ for production (Redis optional)
- DLQ (dead-letter queue) exposed as `queue.dlq` array

## TEST FILES

- `job-queue.test.ts`
- `rate-limiter.test.ts`
- `retry.test.ts`

## NOTES

- Campaign status sync wired in `server.ts` via queue events
- Rate limiter enforces per-platform delays between actions
- Retry policies differ by platform (Facebook more aggressive backoff)
