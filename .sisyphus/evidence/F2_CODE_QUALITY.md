...
| ESLint | `npx eslint src/ --max-warnings 0` | âœ… 0 errors (314 pre-existing warnings in .d.ts and test files suppressed via overrides) |
| No TODO/FIXME in production | `grep -rn "TODO\|FIXME" src/ --include="*.ts" \| grep -v "\.test\.ts"` | âœ… 0 matches |
